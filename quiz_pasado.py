...
def validar_columna(tablero, num_columna):
    ...
    columna = set()
    for fila in range(9):
        columna.add(tablero[fila][num_columna])
...
def validar_subcuadro(tablero, fila_inicio, col_inicio):
    ...
    for i in range(fila_inicio, fila_inicio + 3):
        for j in range(col_inicio, col_inicio + 3):
            cuadro.add(tablero[i][j])
    return cuadro == NUMEROS_VALIDOS
...
def es_subconjunto(conjunto_a, conjunto_b):
    ...
    Retorna True si conjunto_a es subconjunto de conjunto_b.
    """
    actual_a = conjunto_a.cabeza
    while actual_a:
        # Si un elemento de A no pertenece a B, no es subconjunto
        if not conjunto_b.pertenece(actual_a.dato):
            return False
...
def tiene_permisos(permisos_usuario, permisos_requeridos):
    ...
    Retorna True si el usuario tiene TODOS los permisos requeridos.
    """
    # El usuario tiene los permisos si el conjunto de requeridos 
    # es un subconjunto de los permisos que posee el usuario.
...
# PUNTO 1: Encontrar faltantes
def obtener_faltantes(stock_ideal, stock_real):
    """
    stock_ideal: set con nombres de productos que DEBERÍAN estar.
    stock_real: set con nombres de productos que REALMENTE están.
    Retorna: Un conjunto con los productos que están en el ideal pero no en el real.
    """
    # TODO: Implementar
    pass

def obtener_faltantes(stock_ideal, stock_real):
    return stock_ideal - stock_real  # O stock_ideal.difference(stock_real)

# PUNTO 2: Intersección
def interseccion_conjuntos(conjunto_a, conjunto_b):
    """
    Retorna un nuevo objeto de tipo Conjunto que contenga solo
    los elementos que aparecen en AMBOS conjuntos.
    """
    nuevo_conjunto = Conjunto()
    # TODO: Implementar recorriendo conjunto_a y verificando en conjunto_b
    pass
...
# PUNTO 3: Unión de conjuntos
def unir_estudiantes(curso_python, curso_java):
    """
    curso_python: set de nombres.
    curso_java: set de nombres.
    Retorna: Un conjunto con todos los estudiantes de ambos cursos.
    """
    # TODO: Implementar
    pass

def unir_estudiantes(curso_python, curso_java):
    return curso_python | curso_java  # O curso_python.union(curso_java)

# PUNTO 4: Detectar duplicados en fila
def tiene_duplicados(tablero, num_fila):
    """
    Retorna True si en la fila hay números repetidos (ignora el 0).
    Ejemplo: [1, 2, 2, 0, 4] -> True
    """
    vista = set()
    # TODO: Implementar
    pass
...
def tiene_duplicados(tablero, num_fila):
    numeros = [x for x in tablero[num_fila] if x != 0]
    return len(numeros) != len(set(numeros))

# PUNTO 5: Igualdad estricta
def son_iguales(conjunto_a, conjunto_b):
    """
    Retorna True si A y B tienen el mismo tamaño y todos los elementos
    de A están en B.
    """
    # TODO: Implementar
    pass
...
def son_iguales(conjunto_a, conjunto_b):
    if conjunto_a.tamaño != conjunto_b.tamaño:
        return False
    return es_subconjunto(conjunto_a, conjunto_b)